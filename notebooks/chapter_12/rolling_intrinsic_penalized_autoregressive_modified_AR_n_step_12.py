...
class FCQV(nn.Module):
    ...
    def load(self, experiences):
        states, actions, rewards, new_states, is_terminals, gamma_ns = experiences
        states = torch.from_numpy(states).float().to(self.device)
        actions = torch.from_numpy(actions).float().to(self.device)
        new_states = torch.from_numpy(new_states).float().to(self.device)
        rewards = torch.from_numpy(rewards).float().to(self.device)
        is_terminals = torch.from_numpy(is_terminals).float().to(self.device)
        gamma_ns = torch.from_numpy(gamma_ns).float().to(self.device)
        return states, actions, rewards, new_states, is_terminals, gamma_ns
...
class DDPG():
    ...
    def optimize_model(self, idxs_weights_samples):
        ...
        states, actions, rewards, next_states, is_terminals, gamma_ns = experiences
        batch_size = len(is_terminals)
        ...
        # target_q_sa = rewards + self.gamma * max_a_q_sp * (1 - is_terminals)
        target_q_sa = rewards + gamma_ns * max_a_q_sp * (1 - is_terminals)
    ...
    def interaction_step(self, state, env):
        ...
        one_step = (state, action, reward, new_state, float(is_failure))
        self.n_step_buffer.append(one_step)

        # once enough steps are accumulated, store one aggregated transition
        if len(self.n_step_buffer) == self.n_step:
            transition = self._make_n_step_transition()
    ...
    def _make_n_step_transition(self):
        """
        Builds one aggregated n-step transition from the leftmost element
        of self.n_step_buffer.
        Each item in buffer is: (state, action, reward, next_state, done)
        """
        R = 0.0
        gamma_n = 1.0
        ...
        next_state = self.n_step_buffer[0][3]
        done = self.n_step_buffer[0][4]

        for i, (_, _, reward, n_state, d) in enumerate(self.n_step_buffer):
        ...
        return (state, action, R, next_state, float(done), gamma_n)

    def _flush_n_step_buffer(self):
        """
        Flush remaining partial transitions at episode end.
        """
...
torch.save(best_agent.online_policy_model.state_dict(), "online_policy_model_autoregressive_penalized_modified_n_step_12.pth")
torch.save(best_agent.online_value_model.state_dict(), "online_value_model_autoregressive_penalized_modified_n_step_12.pth")
...
plt.savefig("Moving_Average_Reward_Autoregressive_Penalized_modified_n_step_12.png")
...
plt.legend()
plt.grid(True)
plt.savefig("Reinforcement_Learning_Value_Autoregressive_Penalized_modified_n_step_12.png")