...
# v3.1 is updated environment version where s_0 is not fixed
...
class TTFGasStorageEnv(gym.Env):
    def __init__(self, params):
        ...
        self.randomize_s0 = params['randomize_s0']
        self.s0_spot_vol = params['s0_spot_vol']
        self.s0_r_vol = params['s0_r_vol']
        self.s0_delta_vol = params['s0_delta_vol']
        self.s0_v_vol = params['s0_v_vol']
    ...
    def _sample_initial_latent_state(self):
        """
        Sample a randomized initial latent state, then build F_0 from it.
        Keeps month=0 and day=0 unchanged.
        """
        # Spot: use multiplicative noise to keep positivity
        spot_shock = self.W.normal(0.0, self.s0_spot_vol)
        self.S_t = self.initial_spot_price * np.exp(spot_shock)
    
        # Short rate: additive noise, clipped to stay nonnegative
        self.r_t = max(0.0, self.initial_r + self.W.normal(0.0, self.s0_r_vol))
    
        # Convenience yield: additive noise
        self.delta_t = self.initial_delta + self.W.normal(0.0, self.s0_delta_vol)
    
        # Variance: additive noise, clipped to stay nonnegative
        self.v_t = max(0.0, self.initial_v + self.W.normal(0.0, self.s0_v_vol))
        
    def reset(self):
        self.month = 0
        self.day = 0

        if self.randomize_s0:
            self._sample_initial_latent_state()
        else:
...
torch.save(best_agent.online_policy_model.state_dict(), "online_policy_model_autoregressive_td3_v3.1.pth")
torch.save(best_agent.online_value_model_1.state_dict(), "online_value_model_1_autoregressive_td3_v3.1.pth")
torch.save(best_agent.online_value_model_2.state_dict(), "online_value_model_2_autoregressive_td3_v3.1.pth")
...
plt.savefig("Moving_Average_Reward_Autoregressive_Penalized_td3_v3.1.png")
...
plt.legend()
plt.grid(True)
plt.savefig("Reinforcement_Learning_Value_Autoregressive_Penalized_td3_v3.1.png")