...
params = {
    ...
    'penalty_lambda2': 0.0,#1,#10.0,#1.0,#50.0,
...
def intrinsic_policy(state):
    
    I_max = 0.4   # max injection
    W_max = 0.4   # max withdrawal
    V_max = 1.0
    V_min = 0.0
    V_T = 0.0

    t = int(state[0])                # current month
    V_t = state[-1]                  # current storage level
    F_t = state[1:-1]                # e.g., 12-month forward prices
    ...
    b_eq = np.array([V_T - V_t])
    bounds = [(-W_max, I_max)] * n
    zero_price_indices = (F_t == 0)
    adjusted_bounds = [(0, 0) if zero_price_indices[k] else bounds[k] for k in range(len(F_t))]

    return linprog(F_t, A_ub=A, b_ub=b, A_eq=A_eq, b_eq=b_eq, bounds=adjusted_bounds,  method="highs").x
...
for episode in range(5000):
    ...
    ri_action = intrinsic_policy(state)
    state, reward, done, _, _ = env.step(ri_action)
    while not done:
        ri_action = intrinsic_policy(state)  # Your existing RI logic
        ri_dataset.append((state, ri_action))
        state, reward, done, _, _ = env.step(ri_action)

states = torch.tensor([s for s, _ in ri_dataset], dtype=torch.float32)
actions = torch.tensor([a for _, a in ri_dataset], dtype=torch.float32)
...
torch.save(actor.state_dict(), "fcdp_actor_ri-2.pth")