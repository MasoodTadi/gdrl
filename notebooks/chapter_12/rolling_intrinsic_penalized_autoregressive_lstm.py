...
class FCQV(nn.Module):
    def __init__(self, 
        ...
        self.lstm = nn.LSTM(input_size=1,
                            ...
                            num_layers=1,
                            batch_first=True)
        # Input layer: [month, futures_feat, storage, action_dim]
        first_dim = 1 + lstm_hidden_size + 1 + output_dim
        
        #self.input_layer = nn.Linear(input_dim, hidden_dims[0])
        self.input_layer = nn.Linear(first_dim, hidden_dims[0])
    ...
    def _format(self, state, action):
        ...
        if not isinstance(u, torch.Tensor):
            u = torch.tensor(u, 
                             ...
                             dtype=torch.float32)
            u = u.unsqueeze(0)  
        return self._normalize(x), u

    def forward(self, state, action):
        x, u = self._format(state, action)
        month   = x[:, 0:1]       # already normalised by _normalize
        futures = x[:, 1:13].unsqueeze(-1)
        storage = x[:, 13:14]
        lstm_out, (h_n, c_n) = self.lstm(futures)
        futures_feat = h_n[-1]
        # Concatenate state features and action
        x = torch.cat([month, futures_feat, storage, u], dim=1)
...
class FCDPAutoregressive(nn.Module):
    def __init__(self, 
        ...
        # LSTM to encode the 12 futures prices.  It receives sequences of shape (batch, seq_len, input_size)
        # Here input_size=1 because each futures price is a scalar.
        self.lstm = nn.LSTM(input_size=1,
                            hidden_size=lstm_hidden_size,
                            num_layers=1,
                            batch_first=True)
        # New input layer: takes [month (1), LSTM_hidden_size, storage (1)] as input
        first_dim = 1 + lstm_hidden_size + 1
        self.input_layer = nn.Linear(first_dim, hidden_dims[0])
        ...
        # Output heads for each action dimension except the last one (which is computed separately)
        self.output_heads = nn.ModuleList([
            nn.Linear(hidden_dims[-1] + 1, 1) for _ in range(len(self.env_max) - 1)
        ])
    ...
    #         state = state.unsqueeze(0)
    #     # separate components
    #     month = state[:, 0:1] / 11.0        # normalize month
    #     futures = state[:, 1:13].unsqueeze(-1)  # shape (batch, 12, 1)
    #     storage = state[:, 13:14]           # storage level (already normalized outside)
    #     return month, futures, storage

    def forward(self, state):
        state = self._format(state)
        month   = state[:, 0:1]       # already normalised by _normalize
        futures = state[:, 1:13].unsqueeze(-1)
        storage = state[:, 13:14]
        
        batch_size = state.size(0)
        t = (month.squeeze(1) * 11).long()
        # t = (month * 11).long()
        V_t = storage.squeeze(1) * self.V_max  # De-normalize

        #x = self.activation_fc(self.input_layer(state))
        # LSTM encoding; only the last hidden state is used
        lstm_out, (h_n, c_n) = self.lstm(futures)
        # h_n has shape (num_layers, batch, hidden_size); take the last layer
        futures_feat = h_n[-1]             # shape (batch, lstm_hidden_size)
        # Concatenate month, futures feature and storage
        x = torch.cat([month, futures_feat, storage], dim=1)
        x = self.activation_fc(self.input_layer(x))
        
        # Pass through the hidden FC layers
        ...
        for j in range(11):
            # mask = (j + 1 >= t).float().unsqueeze(1)
            mask = (j + 1 >= t).float()           # also shape (batch,)
