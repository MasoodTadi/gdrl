...
class TTFGasStorageEnv(gym.Env):
    def __init__(self, params):
        ...
        # the ORIGINAL risk-neutral dynamics exactly, so nothing else in the file needs a
        # switch.  NOTE: compute_futures_curve() is a Q-pricing functional and is therefore
        # NOT modified - the market still quotes Q-prices, only the state drift differs.
        ...
        # The observation is extended from the original 14 numbers
        #     [month, F^(1..12), V_t]
        # to the 33 numbers built in _build_state() below.  Everything added is a
        # deterministic function of (month, F_t, F_{t-1}, V_t), so no new information
        # enters the problem - it is re-expressed in coordinates (slope, curvature,
        # spread, returns) in which the P-measure drift is a smooth, low-order function.
        # nS is read from observation_space.shape[0] in DDPG.train(), so both networks
        # resize automatically and no other line has to change.
        # Set N_STATE = 14 to recover the original observation.
    ...
    def compute_futures_curve(self):
        ...
        for k in range(12):
            ...
            beta_0 = (self.theta_r / self.sigma_r ** 2) * (
                        2 * np.log(1 - (self.ksi_r - self.kappa_r) * (1 - np.exp(-self.ksi_r * tau)) / (2 * self.ksi_r))
                     ...
                     - (self.sigma_s * self.sigma_delta * self.rho_1 + self.theta_delta) * np.exp(
                -self.kappa_delta * tau) / self.kappa_delta ** 2 \
                     + (4 * self.sigma_delta ** 2 * np.exp(-self.kappa_delta * tau) - self.sigma_delta ** 2 * np.exp(
                -2 * self.kappa_delta * tau)) / (4 * self.kappa_delta ** 3) \
            ...
            F_tk = np.exp(
                np.log(self.S_t) + self.seasonal_factors[k] + beta_0 + beta_r * self.r_t + beta_delta * self.delta_t)
    ...
    def compute_episode_riv(self):
        ...
        for i, tau in enumerate(decision_times):
            ...
            if i < 12:
                X_tau[i] = linprog(prices, A_ub=A, b_ub=b, A_eq=A_eq, b_eq=b_eq, bounds=adjusted_bounds,
                                   method="highs").x
            else:
    ...
    def _build_state(self):
        """Observation handed to the agent.
        ...
        Fbar = 20.0
        t = float(self.month)
        F = np.asarray(self.F_t, dtype=np.float64)
        Fp = np.asarray(getattr(self, 'F_prev', self.F_t), dtype=np.float64)
        if self.N_STATE == 14:
            return np.concatenate(([t], F, [self.V_t]), dtype=np.float32)
        ...
        if alive.any():
            ...
            mx, mn = F[alive].max(), F[alive].min()
        else:
            front = back = mx = mn = 0.0
        mid = (F * alive).sum() / na
        slope = (back - front) / Fbar
        curv = ((front + back) / 2.0 - mid) / Fbar
        spread = (mx - mn) / Fbar if alive.any() else 0.0
        rv = np.abs(ret).sum() / na
        R = max(11 - max(t - 1, 0), 1)
        vrel = self.V_t / (R * self.W_max)
        return np.concatenate((
            [t / 12.0], F / Fbar, [self.V_t], ret,
            [slope], [curv], [spread], [rv],
            [np.sin(2 * np.pi * t / 12.0)], [np.cos(2 * np.pi * t / 12.0)], [vrel]
        ), dtype=np.float32)
    ...
    def step(self, action):
        ...
        F_t = self.F_t
        V_t = self.V_t
        # CHANGE 2: remember the curve observed at the START of this month; it is the
        # F_{t-1} used by the one-month returns in _build_state().
        self.F_prev = F_t
        ...
        for i in range(30):
            ...
            dW_delta = self.rho_1 * dW_1 + np.sqrt(1 - self.rho_1 ** 2) * self.W.normal(0, np.sqrt(
                self.dt))  # For dW_delta (correlated with dW_1)
            dW_v = self.rho_2 * dW_2 + np.sqrt(1 - self.rho_2 ** 2) * self.W.normal(0, np.sqrt(
                self.dt))  # For dW_v (correlated with dW_2)
            ...
            dS_t = (
                               self.r_t - self.delta_t - self.lam * self.mu_j + premium) * self.S_t * self.dt + self.sigma_s * self.S_t * dW_1 + np.sqrt(
                max(self.v_t, 0)) * self.S_t * dW_2 + J * self.S_t * dq
            self.S_t += dS_t
            ...
            dv_t = (self.theta_v - self.kappa_v * self.v_t) * self.dt + self.sigma_v * np.sqrt(
                max(self.v_t, 0)) * dW_v + J_v * dq
            self.v_t += dv_t
...
class FCDPAutoregressive(nn.Module):
    ...
    def _normalize(self, state):
        ...
        # normalisation is applied unchanged, so N_STATE = 14 reproduces the old behaviour.
        if state.shape[1] != 14:
            return state
        state = state.clone()
    ...
    def forward(self, state):
        ...
        # 14-dim: month is col 0 scaled by 11, inventory is the last column scaled by V_max.
        # 33-dim: month is col 0 scaled by 12, inventory is col 13 and is already raw.
        # The action space, the bounds and the autoregressive construction below are
...
class DDPG():
    ...
    def evaluate(self, eval_policy_model, eval_env, n_episodes=1, paired_riv=False):
        # ===================== CHANGE 4: optional paired RIV benchmark =====================
        # With paired_riv=True the rolling-intrinsic value of the SAME realised price path
        # is recorded alongside the agent's P&L, using the environment's own
        # compute_episode_riv().  Common random numbers make the DDPG - RIV difference a
        # paired statistic, which is what the reported standard errors are computed from.
        # Default False, so every existing call site behaves exactly as before.
        rs = []
        rivs = []
        ...
        if paired_riv:
            rs, rivs = np.array(rs), np.array(rivs)
            diff = rs - rivs
...
class NormalNoiseStrategy:
    ...
    def _mask_action(self, action, state):
        ...
        # CHANGE 3: the futures curve is state[1:13] in BOTH layouts.  It was written
        # as state[1:-1] only because the 14-dim observation happened to end with V_t.
        F_t = state[1:13]
        mask = (F_t != 0)
        action[~mask] = 0.0
        return action, mask
    ...
    def select_action(self, model, state, max_exploration=False):
        ...
        # CHANGE 3: read month and inventory by layout, exactly as in the actor.
        # 14-dim: month is state[0] (raw), inventory is the last entry.
        # 33-dim: month is state[0]*12, inventory is state[13].
        if len(state) == 14:
            t = int(state[0])
            V_t = float(state[-1])
        else:
            t = int(round(float(state[0]) * 12))
            V_t = float(state[13])
        action = []
        cum_V = V_t
        for j in range(11):
...
for seed in SEEDS:
    ...
    training_strategy_fn = lambda bounds: NormalNoiseStrategy(bounds, exploration_noise_ratio=0.20,
                                                              final_noise_ratio=0.01,
                                                              max_episode=environment_settings['max_episodes'],
                                                              noise_free_last=0.1 * environment_settings[
                                                                  'max_episodes'])
    ...
    replay_buffer_fn = lambda: PrioritizedReplayBuffer(max_samples=1_000_000,
                                                       batch_size=512)  # (max_samples=200_000, batch_size=256)
    ...
    params = {
        ...
        # phi     : strength of the physical-measure risk premium.  0.0 = original paper.
        #           Measured results: phi = 0, 0.5, 1, 2.
        # n_state : 33 for the extended observation, 14 for the original one.
        'phi': 1.0,
        'n_state': 33,
        # ==================================================================================
    ...
    result, final_eval_score, training_time, wallclock_time = agent.train(env, seed, gamma, max_minutes, max_episodes,
                                                                          goal_mean_500_reward)
...
def compute_futures_curve(day, S_t, r_t, delta_t):
    ...
    for k in range(12):
        ...
        beta_0 = (theta_r / sigma_r ** 2) * (
                    2 * np.log(1 - (ksi_r - kappa_r) * (1 - np.exp(-ksi_r * tau)) / (2 * ksi_r))
                    + (ksi_r - kappa_r) * tau) \
                 ...
                 + (4 * sigma_delta ** 2 * np.exp(-kappa_delta * tau) - sigma_delta ** 2 * np.exp(
            -2 * kappa_delta * tau)) / (4 * kappa_delta ** 3) \
...
for j in range(N_simulations):
    ...
    for i, tau in enumerate(decision_times):
        ...
        if i < 12:
            X_tau[j, i, :] = linprog(prices, A_ub=A, b_ub=b, A_eq=A_eq, b_eq=b_eq, bounds=adjusted_bounds,
                                     method="highs").x
        else:
