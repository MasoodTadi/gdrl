# in this version (update version of AR_3_v5) we make some changes in optimize_model hoping that the critic overestimation is resolved in early steps
...
class DDPG():
    def __init__(self, 
        ...
        self.fixed_a0 = np.array([-0., -0., 0.4, 0.4, 0.2, -0., -0., -0., -0.2, -0.4, -0.4, -0.], dtype=np.float32)
    ...
    def optimize_model(self, idxs_weights_samples):
        ...
        #############################################################################################################################################
        month = states[:, 0].view(-1, 1)

        month_weight = torch.ones_like(td_error)
        month_weight = torch.where(month == 1, 20.0 * month_weight, month_weight)
        month_weight = torch.where(month == 2, 8.0 * month_weight, month_weight)
        month_weight = torch.where(month == 3, 3.0 * month_weight, month_weight)
        # Added: normalized weighted critic loss
        critic_weights = weights * month_weight

        value_loss = (td_error.pow(2) * critic_weights).sum() / critic_weights.sum().clamp(min=1e-8)
    ...
    def train(self, env, seed, gamma,
        ...
        for episode in range(1, max_episodes + 1):
            ...
            # ---- fixed first action a0: move from s0 to s1, but do NOT store transition ----
            state, reward, is_terminal, is_truncated, info = env.step(self.fixed_a0)
            self.episode_reward[-1] += reward
            self.episode_timestep[-1] += 1
            self.episode_exploration[-1] += 0.0   # no exploration on fixed a0
    ...
    def evaluate(self, eval_policy_model, eval_env, n_episodes=1):
        ...
        for _ in range(n_episodes):
            ...
            # fixed first action a0
            s, r, d, _, _ = eval_env.step(self.fixed_a0)
            rs[-1] += r
...
plt.savefig("Moving_Average_Reward_AR_3_v5.png")
...
plt.legend()
plt.grid(True)
plt.savefig("Reinforcement_Learning_Value_AR_3_v5.png")

torch.save(best_agent.online_policy_model.state_dict(), "online_policy_model_AR_3_v5.pth")
torch.save(best_agent.target_policy_model.state_dict(), "target_policy_model_AR_3_v5.pth")
torch.save(best_agent.online_value_model.state_dict(), "online_value_model_AR_3_v5.pth")
torch.save(best_agent.target_value_model.state_dict(), "target_value_model_AR_3_v5.pth")