...
class FCDP(nn.Module):
    ...
    def forward(self, state):
        ...
        month = x[:, 0].long()  # shape: (batch_size,)
        ...
        actions = self.rescale_fn(x)  # actions shape: (batch_size, num_actions)
...
class NormalNoiseStrategy:
    ...
    def select_action(self, model, state, max_exploration=False):
        ...
        final_action, mask = self._mask_action(clipped_action, state)
        self._compute_noise_diagnostics(final_action, greedy_action, mask)
...
plt.legend()
plt.grid(True)
plt.savefig("Reinforcement_Learning_Value_Penalization_non_projected.png")
...
plt.legend()
plt.grid(True)
plt.savefig("Reinforcement_Learning_Value_Penalization_Projected.png")